...
void describe('ServiceBootstrap E2E', () => {
  ...
  void it('initializes and connects to emulators', async () => {
    const ctx = await bootstrap!.initialize();

    assert.ok(ctx.firestore);
    assert.ok(ctx.pubsub);
    assert.ok(ctx.eventStore);
  });
});
